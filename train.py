...
def train_model(config):
    ...
    global_step = 0
    if config['preload'] and config['preload'].startswith("hf://"):
        optimizer.load_state_dict(torch.load(f"{config['model_folder']}/optimizer.pt"))
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        # CHANGE: Save model and tokenizers to Hugging Face Hub
        save_model_to_hub(model, tokenizer_src, tokenizer_tgt, epoch, config)

        # Save optimizer state
        torch.save(optimizer.state_dict(), f"{config['model_folder']}/optimizer.pt")
...
if __name__ == '__main__':
    ...
    # CHANGE: Initialize wandb for experiment tracking
    wandb.init(
